 ...
 *   node scripts/generate-chapter-banners.js --execute    # submit jobs
 *   node scripts/generate-chapter-banners.js --poll       # poll + download
 *   node scripts/generate-chapter-banners.js --list       # status
 *   node scripts/generate-chapter-banners.js --force      # re-generate failed
 *   node scripts/generate-chapter-banners.js --track 5    # single track
 */
...
const { buildChapterBannerPrompt, SONG_CHAPTERS } = require('./lib/prompts')
...
function writeManifest(state) {
  ...
  log(`Manifest: ${entries.length} chapter banners.`)
}
...
async function runExecute(tracks) {
  ...
  for (const t of tracks) {
    ...
    const prompt = buildChapterBannerPrompt(t.trackNumber, t.title)
  ...
  log(`\n✅  Submitted ${submitted} chapter banner job(s).`)
}
...
async function runPoll() {
  ...
  else log('All chapter banners complete!')
  writeManifest(state)
}

function runList(tracks) {
  const state = loadState(OUTPUT_DIR)
  console.log('\n🖼  Chapter banner status:\n')
  for (const t of tracks) {
    const e = state[t.id]
...
function runDryRun(tracks) {
  console.log('\n🖼  DRY RUN — Chapter banner prompts\n')
  for (const t of tracks) {
    const prompt = buildChapterBannerPrompt(t.trackNumber, t.title)
    console.log(`\n▶  Track ${t.trackNumber}: "${t.title}"\n${prompt.slice(0,200)}…`)
  }
  console.log(`\n📊  ${tracks.length} banner(s) — use --execute to generate.\n`)
}
