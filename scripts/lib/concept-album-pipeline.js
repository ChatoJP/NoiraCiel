'use strict'
/**
 * concept-album-pipeline.js
 * Shared, manifest-driven generation pipeline for new NoiraCiel concept
 * albums (unlike the older generate-*-albums.js scripts, which hardcode one
 * album's track list per file, this reads any album from
 * data/concept-albums/{slug}/{concept,tracks,status}.json).
 *
 * Hard rule (explicit user mandate, repeated twice): one heavy asset at a
 * time. generate -> download to temp -> upload to R2 -> verify -> update
 * manifest -> delete temp -> only then move to the next asset. If upload or
 * verification fails, stop the batch immediately and surface the error —
...
const ALBUMS_DIR = path.join(ROOT, 'data', 'concept-albums')
const TMP_DIR    = path.join(ROOT, '.tmp', 'concept-albums')
...
/** Ensure every track has a status entry; idempotent. */
  ...
  for (const t of tracks) {
    if (!status[t.slug]) {
      status[t.slug] = {
        ...
        music: { taskId: null, state: 'none', audioUrl: null, r2Key: null },
        songArt: { taskId: null, state: 'none', r2Key: null, url: null },
        chapterBanner: { taskId: null, state: 'none', r2Key: null, url: null },
        error: null,
...
/** Suno's style field has a hard 200-char limit; trim at a comma boundary
 *  rather than mid-word so the truncated prompt still reads cleanly. */
...
async function runMusicForTrack(albumSlug, track, status) {
  ...
  // Dead taskId from a genuinely failed job — resubmit instead of re-polling.
  ...
  if (!entry.music.taskId) {
    r2.log(`[music] ${track.title} — submitting Suno job`)
  ...
  // Upload to R2, verify, update manifest, delete temp — strictly in that order.
  const r2Key = `music/${concept.dirName}/audio/${numStr}_${track.slug}.mp3`
...
const IMAGE_KIND_CONFIG = {
  songArt:       { aspectRatio: '1:1',  r2Prefix: 'images/song-art' },
  chapterBanner: { aspectRatio: '16:9', r2Prefix: 'images/chapter-banners' },
}
...
async function runImageForTrack(albumSlug, track, status, kind) {
  ...
  // A taskId from a job that actually failed (not just a timed-out poll) is
  // dead — re-polling it forever returns the same failure. Clear it so the
  // retry below resubmits a fresh job instead of looping on a dead one.
  ...
  entry.error = null
  if (entry.songArt.state === 'complete' && entry.chapterBanner.state === 'complete') {
